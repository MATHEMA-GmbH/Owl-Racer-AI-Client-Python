...
def mainLoop(args):
    ...
    label_map_path = os.path.join(os.path.dirname(model_name), "labelmap_dev.yaml")
    ...
    while True:
        ...
        ### dev
        #print(f"output probabilities: {action}")
        ### \dev

        # action = idx2class[np.argmax(action)]

        ### dev
        action = idx2class[np.random.choice([0,1,2,3,4], p=action[0])]
        ### \dev
