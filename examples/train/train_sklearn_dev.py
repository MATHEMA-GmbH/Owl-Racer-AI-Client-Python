...
from owlracer_dataset_dev import OwlracerPreprocessor
...
def get_features(drop_columns: list = None):
    preprocessor = OwlracerPreprocessor(data_path=data_path)
    preprocessor.clean_crashes()
    [used_columns, used_commands, replace_commands] = preprocessor.change_datatype(drop_columns)
...
if __name__ == '__main__':
    ...
    class2idx = yaml.safe_load(open("examples/train/labelmap_dev.yaml"))["class2idx"]
    ...
    drop_columns = ["Time", "Id", "IsCrashed", "MaxVelocity", "Position.X", "Position.Y",
                    "Checkpoint", "Rotation", "Score", "ScoreOverall", "Ticks",
                    "ScoreChange", "WrongDirection"]

    # --- ---
    used_columns, used_commands, replace_commands, X_train, X_test, Y_train, Y_test = get_features(drop_columns)
    # --- \ ---
